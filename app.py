...
def main():
    ...
    survived_count = df_processed['Survived'].sum()
    survival_rate = (survived_count / total_passengers * 100) if total_passengers > 0 else 0
    
    metric_col1, metric_col2, metric_col3, metric_col4 = st.columns(4)
    
    with metric_col1:
        st.metric("Всего пассажиров", total_passengers)
    
    with metric_col2:
        st.metric("Выжившие", survived_count)
    
    with metric_col3:
        st.metric("Погибшие", total_passengers - survived_count)
    
    with metric_col4:
        st.metric("Выживаемость", f"{survival_rate:.1f}%")
    ...
    if not df_processed.empty:
        ...
        # Создание таблицы по портам
        port_details = []
        
        for port in df_processed['Embarked'].unique():
            if pd.notna(port):
                port_data = df_processed[df_processed['Embarked'] == port]
